...
void USART_Printf(char *str)        //发送字符串
{
...
void USART_Send_Nbit_Data(void *data, int Nbit)  //int    long int     float 均为四位  double 8位
{
	uint8_t *temp_pointer = (uint8_t *)data;      //把float指针转为uint8_t指针，便于每8字节读取一个数据，否则则是32位  因为float就是32位的
	for(int i=0;i<Nbit;i++)   //float占用四个字节
	{
		USART_SendByte(*(temp_pointer+Nbit-1-i));   //从高字节开始往低字节发，也就是说先发的是高位
	}  //完毕
}
...
uint8_t USART_Read_Newest_Byte(void)    //获取串口读取到的最新的数据
{
...
void USART_Recv_DataArray_Clear(void)   //清空串口缓冲区数据
{
